...
\begin{itemize}
    \item Vérifier qu'une application est linéaire.
    \item Déterminer noyau, image, rang, injectivité et surjectivité.
    \item Construire une application linéaire à partir de l'image d'une base.
    \item Passer d'une application linéaire à sa matrice, et inversement.
    \item Comprendre les projecteurs, symétries, formes linéaires et changements de base.
    \item Préparer les exercices d'oraux de type Mines, Centrale et CCP-INP.
\end{itemize}
...
\exo{Vérifier une linéarité simple}{APP}{$\star$}
...
\exo{Trouver un contre-exemple à la linéarité}{APP}{$\star$}
...
\]
\begin{enumerate}
    \item Calculer $f(1,0)$ et $f(2,0)$.
    \item Comparer $f((1,0)+(1,0))$ avec $f(1,0)+f(1,0)$.
    \item Conclure que $f$ n'est pas linéaire.
    \item Expliquer pourquoi le terme $x^2$ est incompatible avec la linéarité.
\end{enumerate}

\exo{Forme linéaire dans $\mathbb{R}^3$}{APP}{$\star$}
...
\begin{enumerate}
    ...
    \item Interpréter géométriquement ce noyau.
\end{enumerate}
...
\]
et soit $u$ l'application linéaire de $\mathbb{R}^2$ dans $\mathbb{R}^2$ de matrice $A$ dans la base canonique.
\begin{enumerate}
...
\begin{enumerate}
    ...
    \item Résoudre $u_\lambda(x,y)=(0,0)$ selon les valeurs de $\lambda$.
...
\end{pmatrix}.
\]
On note $u$ l'endomorphisme de $\mathbb{R}^3$ de matrice $A$ dans la base canonique.
\begin{enumerate}
    \item Écrire $u(x,y,z)$.
...
\]
Ces deux vecteurs sont non colinéaires, donc forment une base du noyau. La dimension vaut $2$. Géométriquement, le noyau est un plan vectoriel de $\mathbb{R}^3$.
...
\]
Comme $(1,0)$ et $(0,1)$ forment une base de $\mathbb{R}^2$, on obtient
\[
\Img(u)=\mathbb{R}^2.
\]
Donc
...
\Ker(u)=\Vect((-1,1,-1)).
\]
Le noyau n'est pas nul, donc $u$ n'est pas injective. D'après l'exercice précédent, l'image est $\mathbb{R}^2$, donc $u$ est surjective. Enfin :
\[
...
\end{pmatrix}.
\]
L'application ne peut pas être surjective de $\mathbb{R}^2$ vers $\mathbb{R}^3$, car son rang est au plus $2$.
...
\corr{11}
Une application linéaire est entièrement déterminée par l'image d'une base. Il existe donc une unique application linéaire vérifiant ces conditions.

Pour $(x,y,z)=xe_1+ye_2+ze_3$, on a
\[
...
\end{cases}
\]
Si $\lambda\neq 1$ et $\lambda\neq -1$, la seule solution est $(0,0)$, donc le noyau est nul. L'application est alors injective. Comme elle va de $\mathbb{R}^2$ dans $\mathbb{R}^2$, elle est aussi surjective.
...
\]
Les vecteurs $(1,1)$ et $(1,2)$ ne sont pas colinéaires, donc ils forment une base de $\mathbb{R}^2$. Ainsi
\[
\Img(u)=\mathbb{R}^2,
\qquad
\rg(u)=2.
...
\Ker(u)=\{0\}.
\]
L'application est injective. Comme c'est un endomorphisme de $\mathbb{R}^3$, elle est bijective. Donc
\[
\rg(u)=3,
\qquad
\Img(u)=\mathbb{R}^3.
...
\]
Elle est de dimension $2$. L'application est injective mais non surjective, car $\mathbb{R}_2[X]$ est de dimension $3$.
...
\]
Cette famille est libre, donc elle est une base du noyau. Le noyau est de dimension $3$, c'est donc un hyperplan de $\mathbb{R}^4$.
...
\]
Donc $(u(e_1),\dots,u(e_n))$ engendre $\Img(u)$.
Un exemple où cette famille n'est pas libre : prendre $u:\mathbb{R}^2\to\mathbb{R}$, $u(x,y)=x$. L'image de la base canonique est $(1,0)$ dans $\mathbb{R}$, famille liée.
...
\]
On obtient alors une relation linéaire entre les vecteurs d'une base de $E$, donc tous les coefficients $\lambda_i$ sont nuls. La famille est libre.
...
Réciproquement, si $u$ est surjective, alors $\rg(u)=\dim(F)=\dim(E)$. Le théorème du rang impose alors
\[
\dim(\Ker(u))=0.
\]
...
\Ker(u)=\{0\}.
\]
Comme $u:\mathbb{R}^3\to\mathbb{R}^3$ et que son noyau est nul, $u$ est injective, donc bijective. Par conséquent
\[
\Img(u)=\mathbb{R}^3,
\qquad
\rg(u)=3.
...
p(x)=f.
\]
Alors $p$ est linéaire, $p^2=p$, $\Img(p)=F$ et $\Ker(p)=G$. Dans une base adaptée, sa matrice est une matrice bloc avec des $1$ sur le bloc correspondant à $F$ et des $0$ sur le bloc correspondant à $G$.
