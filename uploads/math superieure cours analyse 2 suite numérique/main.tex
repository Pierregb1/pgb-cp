...
P(n)\sim a_p n^p,
\]
où \(P\) est un polynôme de degré \(p\) et de coefficient dominant \(a_p\neq0\).

Si \(P\) et \(Q\) sont deux polynômes de degrés respectifs \(p\) et \(q\), de coefficients dominants \(a_p\) et \(b_q\), avec \(b_q\neq0\), alors :
\[
\frac{P(n)}{Q(n)}
\sim
...
Pour un polynôme :
\[
P(n)=a_p n^p+a_{p-1}n^{p-1}+\cdots+a_0,
\]
avec \(a_p\neq0\). On factorise par le terme dominant :
\[
P(n)=a_p n^p\left(1+\frac{a_{p-1}}{a_p}\frac1n+\cdots+\frac{a_0}{a_p}\frac1{n^p}\right).
\]
Le facteur entre parenthèses tend vers \(1\). Donc :
\[
\frac{P(n)}{a_p n^p}\to1,
\]
ce qui donne :
...
Le résultat sur le quotient de deux polynômes s'obtient en appliquant cet équivalent au numérateur et au dénominateur.
\end{proof}

\begin{methodbox}[Trouver un équivalent d'une expression rationnelle]
Pour une expression de la forme :
\[
\frac{P(n)}{Q(n)},
\]
on garde uniquement les termes dominants de \(P\) et de \(Q\). Ainsi :
\[
\frac{a_p n^p+\text{termes de plus bas degré}}{b_q n^q+\text{termes de plus bas degré}}
\sim
\frac{a_p}{b_q}n^{p-q}.
\]
\end{methodbox}
...
%---------------------------------------------------------
\subsection{Équivalents avec racines carrées}

\begin{methodbox}[Factoriser le terme dominant sous une racine]
Pour étudier une suite du type :
\[
\sqrt{n^2+an+b},
\]
on factorise \(n^2\) sous la racine :
\[
\sqrt{n^2+an+b}
=
 n\sqrt{1+\frac{a}{n}+\frac{b}{n^2}},
\]
puis on utilise la continuité de la fonction racine carrée en \(1\).
\end{methodbox}

\begin{theorembox}[Racines carrées usuelles]
Lorsque \(n\to+\infty\), on a :
\[
\sqrt{n^2+an+b}\sim n.
\]
...
\sqrt{a n^2+bn+c}
=
n\sqrt{a+\frac bn+\frac c{n^2}}.
\]
Comme :
\[
a+\frac bn+\frac c{n^2}\to a,
\]
et comme \(x\mapsto\sqrt{x}\) est continue en \(a>0\), on obtient :
\[
\sqrt{a+\frac bn+\frac c{n^2}}\to\sqrt a.
\]
Donc :
\[
\sqrt{a n^2+bn+c}\sim \sqrt a\,n.
...
\begin{exercicebox}[Racine carrée]
Donner un équivalent de :
\[
u_n=\sqrt{4n^2+3n+7}.
\]
...
On factorise \(n^2\) sous la racine :
\[
u_n=n\sqrt{4+\frac3n+\frac7{n^2}}.
\]
Le facteur sous la racine tend vers \(4\), donc :
\[
\sqrt{4+\frac3n+\frac7{n^2}}\to2.
\]
Ainsi :
\[
u_n\sim2n.
...
%---------------------------------------------------------
\subsection{Développements asymptotiques très simples}

\begin{theorembox}[Équivalents issus des fonctions usuelles]
Lorsque \(x\to0\), on a :
\[
\sin x\sim x,
\qquad
...
\]
En particulier, lorsque \(n\to+\infty\), on peut poser \(x=\frac1n\) pour obtenir des équivalents de suites.
\end{theorembox}

\begin{exercicebox}[Équivalent trigonométrique]
Donner un équivalent de :
\[
u_n=\sin\left(\frac1n\right).
\]
\end{exercicebox}
...
\begin{exercicebox}[Équivalent logarithmique]
Donner un équivalent de :
\[
v_n=\ln\left(1+\frac3n\right).
\]
\end{exercicebox}
...
on obtient :
\[
\ln\left(1+\frac3n\right)\sim \frac3n.
\]
\end{correctionbox}

%---------------------------------------------------------
\subsection{Règles de calcul avec les équivalents}

\begin{theorembox}[Produit, quotient, puissance]
Soient \((u_n),(v_n),(a_n),(b_n)\) des suites non nulles à partir d'un certain rang.
Si :
\[
u_n\sim v_n
\qquad\text{et}\qquad
...
\]
Si \(k\in\Z\), et si les puissances considérées sont bien définies à partir d'un certain rang, alors :
\[
u_n^k\sim v_n^k.
\]
\end{theorembox}
...
\begin{proof}
On utilise les quotients. Par exemple :
\[
\frac{u_n a_n}{v_n b_n}
=
\frac{u_n}{v_n}\cdot\frac{a_n}{b_n}.
\]
Or chacun des deux facteurs tend vers \(1\). Donc le produit tend vers \(1\), ce qui donne :
\[
u_n a_n\sim v_n b_n.
\]
Le quotient se démontre de la même manière.
\end{proof}

\begin{erreurbox}
Les équivalents se multiplient et se divisent bien, mais ils ne s'additionnent pas toujours.
Par exemple :
\[
n+1\sim n
\qquad\text{et}\qquad
...
n+(-n)=0.
\]
On ne peut donc pas conclure à un équivalent par addition.
\end{erreurbox}

\begin{methodbox}[Éviter les compensations]
Lorsqu'une expression contient une différence de termes équivalents, il faut éviter de remplacer brutalement chaque terme par son équivalent. On utilise plutôt :
\begin{enumerate}
    \item une factorisation ;
    \item la quantité conjuguée ;
    \item un développement limité ;
    \item une mise au même dénominateur.
\end{enumerate}
\end{methodbox}

\begin{exercicebox}[Différence avec racines]
...
\subsection*{Thème 2 -- Monotonie et convergence}
...
\subsection*{Thème 4 -- Comparaisons asymptotiques}
...
Donner un équivalent de :
\[
w_n=\ln\left(1+\frac2n\right)+\sin\left(\frac1n\right).
\]
\end{exercicebox}
...
Déterminer la limite de :
\[
x_n=n\left(\sqrt{1+\frac1n}-1\right).
\]
\end{exercicebox}
...
\]
Soit \(\varepsilon>0\). On choisit \(N\in\N^*\) tel que :
\[
N\geq\frac1\varepsilon.
\]
Alors, pour tout \(n\geq N\),
...
On cherche d'abord la limite éventuelle. Si \(u_n\to\ell\), alors :
\[
\ell=\frac{\ell+3}{2}.
\]
Donc :
...
u_{2n+1}=-1.
\]
Les deux sous-suites convergent vers deux limites différentes. Donc \((u_n)\) ne converge pas.
\end{correctionbox}

\begin{correctionbox}[7]
Soit \(\varepsilon>0\). Il existe \(N_1\) tel que :
\[
n\geq N_1\implique \abs{u_{2n}-\ell}\leq\varepsilon.
\]
Il existe \(N_2\) tel que :
...
N=2\max(N_1,N_2)+1.
\]
Si \(k\geq N\), alors \(k\) est soit pair, soit impair. Dans les deux cas, il s'écrit :
\[
k=2n
\quad\text{ou}\quad
k=2n+1
...
Donc :
\[
u_n\sim\frac{2n^3}{7n^2}=\frac27 n.
\]
\end{correctionbox}
...
\sin\left(\frac1n\right)\sim\frac1n.
\]
Ici il n'y a pas compensation puisque les deux termes sont positifs à partir d'un certain rang. On obtient donc :
\[
w_n\sim\frac2n+\frac1n=\frac3n.
\]
\end{correctionbox}
...
x_n=n\left(\sqrt{1+\frac1n}-1\right)
=
n\frac{\left(1+\frac1n\right)-1}{\sqrt{1+\frac1n}+1}.
\]
Donc :
\[
x_n=
...
Montrons qu'elle est majorée. Pour tout \(k\geq2\),
\[
...
<2.
\]
Donc \((u_n)\) est majorée par \(2\). Elle est croissante et majorée, donc convergente.
\end{correctionbox}
...
u_n\to m.
\]
Soit \(\varepsilon>0\). À partir d'un certain rang :
\[
...
\abs{u_n}\leq\abs{\ell}+1.
\]
Les premiers termes étant en nombre fini, ils sont bornés. La suite entière est donc bornée.
...
f(x)=\frac{x+1}{2}
\]
envoie \([0,1]\) dans \([0,1]\). En effet, si :
\[
0\leq x\leq1,
\]
alors :
...
\textbf{4.}
Soit \(\ell\) sa limite. Par continuité de \(f\), on obtient :
\[
\ell=\frac{\ell+1}{2}.
\]
Donc :
...
\textbf{2.}
Comme \((u_n)\) est bornée, le théorème de Bolzano-Weierstrass s'applique directement. Donc il existe une extractrice \(\varphi:\N\to\N\) strictement croissante et un réel \(\ell\) tels que :
\[
u_{\varphi(n)}\to\ell.
\]
...
\subsection*{13. Règles avec les équivalents}

On peut multiplier, diviser et élever à une puissance des équivalents, mais il faut se méfier des additions et des soustractions à cause des compensations.

%=========================================================
\section*{Conclusion pédagogique}
\addcontentsline{toc}{section}{Conclusion pédagogique}

Les suites numériques constituent le premier outil fondamental de l'analyse. Elles permettent de comprendre rigoureusement la notion de limite et préparent directement :
\begin{itemize}
...
\begin{itemize}
    ...
    \item éviter les additions d'équivalents lorsqu'il peut y avoir compensation.
\end{itemize}
